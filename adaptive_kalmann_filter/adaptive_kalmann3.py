...
if __name__=='__main__':
    ...
    IC_T=IC.set_index('tradeDate')
    ICdelay = ICIRDelay(IC, delay=60)
    ...
    # 自适应kalmann滤波预测
    mu=0.9
    IC_pre=adaptiveKalmannFilterIteration(ICdelay,y_IC,mu=mu,sigma1=0.00001,sigma2=0.00001)
    len=len(IC)
    r_square=1-np.sum((IC.iloc[:,1]-IC_pre.iloc[:,0])**2)/np.sum((IC.iloc[:,1]-IC.iloc[:,1].mean())**2)
    adj_r_square=(1-(1-r_square**2)*(len-1))/(len-660-1)
    ...
    ax.plot(dates, avgIC_pre.iloc[:, 0].values, label='avgIC_prediction')
    ...
    avgIC = factor_combination(factors, avgIC_pre, 'avgIC')
    IR = factor_combination(factors, IR_pre, 'IR')
    
    
    # 各个组合因子合并
    fac_comb = pd.concat([avgIC, IR], axis=1)
    '''
