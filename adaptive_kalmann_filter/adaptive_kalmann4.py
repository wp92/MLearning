...
def ICIRDelay(data, delay=60):  # 用于avgIC 和IR
...
def factor_combination(factors, weight, fac_comb_name='fac_com'):
    ...
    for i in range(len(dates)):
        if i<len(dates)-1:
            fac_comb.loc[idx[dates[i], :], fac_comb_name] = factors[factor].loc[idx[dates[i], :], :].values.dot(weight.loc[dates[i+1], :].values)

    fac_comb=fac_comb.reset_index()
    fac_comb=fac_comb[fac_comb['tradeDate']!=dates.pop(-1)]
    fac_comb=fac_comb.set_index(['tradeDate', 'secID'])
    return fac_comb
...
def adaptiveKalmannFilterIteration(phi, y, mu=0.1, sigma1=0.0001, sigma2=0.0001):
    ...
    for j in range(y.shape[1]):
        for i in range(phi.shape[0] - 1):
            ...
            y_hat_total.iloc[i + 1, j] = y_hat[0][0]*3
            ...
            P = P - mu / dom * P.dot(phi.iloc[i + 1, :].values.reshape(n, 1)).dot(
                phi.iloc[i + 1, :].values.reshape(1, n)).dot(
                P) + mu * Q
    ...
    for fac in factors:
        for i in range(len(dates)):
            if i < avgnum:
                ...
                avgIC.loc[dates[i], fac + '_avgIC'] = np.dot(data.iloc[range(i + 1 - avgnum, i + 1), :][fac].values,
                                                             avgwww)
...
if __name__ == '__main__':
    if sys.platform == 'win32':
        ...
        fileloc = '/root/Documents/.file/data/'
    ...
    # avgIC、IR 延时
    IC = ICIR.loc[:, ICList].set_index('tradeDate').reset_index()


    delaynum=5
    ICdelay = ICIRDelay(IC, delay=delaynum)
    # ICdelay.to_csv('C:/Users/Administrator/Desktop/avgICdelay.csv')
    #
    y_IC = IC.iloc[1:,:]
    ...
    for i in range(len(ICList)):
        iclist=['tradeDate']
        l=len(ICList[i])
        for j in range(len(ICdelay.columns)):
            if ICdelay.columns[j][:l]==ICList[i]:
                iclist.append(ICdelay.columns[j])
        pre = adaptiveKalmannFilterIteration(ICdelay.reset_index().loc[:,iclist].set_index('tradeDate'), y_IC.set_index('tradeDate').iloc[:,i].reset_index(), mu=mu, sigma1=sigma1, sigma2=sigma2)
        IC_pre=pd.concat([IC_pre,pre],axis=1)

    #print(pd.concat([IC.set_index('tradeDate').iloc[:,0],IC_pre.iloc[:,0]],axis=1))
    #预测结果分析

    length = len(IC)
    r_square = 1 - np.sum((IC.iloc[10:, 1].values - IC_pre.iloc[10:, 0].values) ** 2) / np.sum(
        (IC.iloc[10:, 1] - IC.iloc[:, 1].mean()) ** 2)
    adj_r_square = 1 - (1 - r_square ** 2) * (length - 1) / (length - delaynum - 1)
    print(r_square, adj_r_square)

    #计算avgIC、IR
    #avgIC_pre,IR_pre=avgIC_IR_caculation(IC_pre,avgnum=60)
    ...
    #print(np.sum((ICIR.loc[:,avgICList].set_index('tradeDate').iloc[:,0].values-avgIC_pre.iloc[:,0].values)**2))


    dates=ICdelay.reset_index()['tradeDate']
    fig, ax = plt.subplots()
    ax.plot(dates[10:], IC.set_index('tradeDate').iloc[10:,0].values, label='IC_initial')
    ax.plot(dates[10:], IC_pre.iloc[10:,0].values, label='IC_prediction')
    ...
    #开始组合因子
    print('Factors combination begin!')
    fac_comb = factor_combination(factors, IC_pre, 'avgIC')
    exit()
    # 保存
