# -*- coding=utf-8 -*-
#数据准备icir表及所有因子暴露值表，返回预测的avgIC 和 IR
import numpy as np
import pandas as pd
...
if __name__ == '__main__':
    ...
    dataname = ['avgICdelay_lr_pre', 'IRdelay_lr_pre']
    ...
    for j in range(len(data)):
        ...
        for k in range(len(columns)):  # 因子
            ...
            for i in range(num + 1):
                ...
                x_train, y_train = data[j].loc[train_d[:-1], :].values, data[j].loc[train_d[1:], :][data_columns[j][k]].values
