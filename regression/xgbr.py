...
class Linear:
    ...
    def learning(self, data, method='linear', length=250, rollingnum=30,
                 splits={'splits': [0.8, 0, 0.2]}):  # length每次滚动序列的长度，rollingnum滚动的长度
        ...
        for k in range(len(self.factor)):  # 因子
            ...
            for i in range(num + 1):
                ...
                x_test, y_test = data.loc[test_d[:-1], :].values, data.loc[test_d[1:], :][data_columns[k]].values
                x_predict = data.loc[prediction_d, :].values

                model = xbr(max_depth=3, n_estimators=10, objective='reg:linear', booster='gbtree')
...
if __name__ == '__main__':
    ...
    factors = pd.read_csv(fileloc + 'allfactors3_20190409.csv').set_index(['tradeDate', 'secID']).iloc[:,
              2:].reset_index()
    ...
    # 线性回归预测
    avgIC_pre = linear.learning(avgICdelay, method='linear', length=250, rollingnum=30,
                                splits={'splits': [0.64, 0.16, 0.2]})
    IR_pre = linear.learning(IRdelay, method='linear', length=250, rollingnum=30, splits={'splits': [0.64, 0.16, 0.2]})
    ...
    # 计算因子方向
    factor_d = linear.factor_direction(pd.concat([ICIR.loc[:, 'tradeDate'], ICIR.loc[:, linear.factor_ic]], axis=1),
                                       length=252)
    ...
    fac_comb.to_csv(fileloc + 'fac_comb_xgb_{}.csv'.format(nowdate))
